...
export const investigationV2StoryletSeeds: InvestigationStoryletSeed[] = [
  ...
  },
  {
    ...
    check: {
      ...
      baseDifficulty: 10,
      coopAllowed: true,
      assistAttributes: ["insight", "lore"],
    },
    results: {
      revelation:
        "你在比对噩梦报告时，灵性感知捕捉到了一个不该出现的规律：噩梦出现的时间不是随机的——它们集中在每天凌晨两点到三点之间，恰好是旧教堂封存记录上标注的那次不完整激活发生的时刻。不仅如此，所有噩梦报告者都住在石碑灵性辐射的同心圆区域内。你看到了辐射的扩散地图——以教堂为中心，两个街区外的噩梦内容更模糊，四个街区外的人完全不记得梦见了什么。这证明噩梦的源头是精确的、定向的——不是大气中弥漫的污染，而是一个位于地下的、有指向性的灵性来源。",
      success:
        "比对证人记录后，你的直觉指向了一个明确的方向：噩梦的核心内容——'在等待什么的空洞回声'——不是随机产生的恐惧意象。多名证人在各自的梦里都听到了一组相同的音节，你用赫密斯语的基础知识拼出了一个词：'遗忘'。噩梦不是无意义的——它在传递同一个信息，只是大部分人听不懂。",
      partial:
        "你能确认噩梦的相似性不是巧合——多名互不相识的居民描述了近乎相同的梦境结构。但噩梦的具体内容超出了你能解读的范围。有证人提到梦里的声音是'一种听不懂的话'——可能是某种已失传的语言，也可能是纯粹的无意义音节。你需要一个懂仪式学或语言的人来确认。",
      failure:
        "噩梦报告在你手中像一堆互相矛盾的碎片。有人梦到了教堂钟声，有人梦到了石碑，有人只记得一种'被什么东西看着但看不见它'的感觉。你无法确定这些噩梦之间是否存在真实的关联——还是人们在大雾霾的焦虑中各自产生的幻觉。线索不足。",
      catastrophe:
        "你在翻阅证人噩梦记录时突然产生了一个不该出现的念头：你觉得自己也做过那个梦。你从来没有来过大雾霾后的东区——但你闭上眼睛的时候，竟然能准确描述出教堂地下室的结构。那是刚才从证词里看到的吗？还是你在翻看这些报告的时候，石碑的辐射已经在你不知道的时候影响了你的记忆？污染+2。你的分析被恐惧打断——你不确定自己读到的证据里，哪些是真实的，哪些是你被植入的。",
    },
  },
  {
  ...
  },
  {
    ...
    check: {
      ...
      baseDifficulty: 14,
      coopAllowed: true,
      assistAttributes: ["lore", "cunning"],
    },
    results: {
      revelation:
        "你把慈善项目的物资分发点、室内活动场地和居民流动路线叠在同一张地图上的时候，图案出现了。不是随机的服务网络——是一张精确的引导网络。每个分发点、每个活动场地都恰好位于灵性敏感度高于平均值的居民住所附近。不仅如此——每个地点之间的距离几乎相等，形成了一个近似六边形的网格。这不是慈善运营——这是工程。布置这张网的人知道每一个节点的位置应该在哪里，不是因为他了解东区的贫困人口分布，而是因为他了解每一个高灵性敏感个体的精确住址。这张网是用来引导灵性的——而不只是引导人们的脚步。",
      success:
        "逐个比对物资分发点、活动场地和证人住所后，一个模式浮现了出来：不是随机的——每个地点都正好位于至少一名灵性敏感度偏高的居民的日常活动路线上。而且每两条路线在教堂方向上的交汇点恰好构成了一条连续的通道。普通的慈善项目运营不会产生这么精准的地理重合。有人在利用慈善项目选点。",
      partial:
        "你注意到几个地点之间的间距和排列有些不自然——太整齐了。但你没有完整的居民灵性敏感度数据和所有分发点的精确分布图。你能确认这个模式存在，但无法判断它到底是蓄意设计的还是东区街道自然形成的格局。需要更多的地图和居民档案来确证。",
      failure:
        "分发点和居民住所的分布在你眼中像一幅没有标题的地图。你能看到一些地点之间距离很近，但看不出统一的规律。东区的街巷本来就像迷宫——任何人在纸面上连线都可能发现一些巧合。你需要一个对城市格局更熟悉的人帮忙。",
      catastrophe:
        "你在地图上标记慈善点位的时候，突然注意到一件事：你自己的住所在桥区的哪个方向？你想不起来。你记得街道的名字，但不记得它是朝东还是朝西。你把东区地图翻到另一面——你的注意力被教堂街的位置吸住了。不，不是吸住了——是滑过去了。像一个你明知不该去但又不知不觉朝那个方向挪了一小步的冲动。污染+2。你的分析被打断了。你无法继续比对地图——因为你现在不确定你在地图上看到的位置偏差是你的错觉还是真实存在的。",
    },
  },
  {
  ...
  },
  {
    ...
    check: {
      ...
      baseDifficulty: 14,
      coopAllowed: true,
      assistAttributes: ["lore", "insight"],
    },
    results: {
      revelation:
        "你用灵性感知扫过三件封印物的内部铭文结构时，发现它们的鉴定系统中都嵌有一段相同的赫密斯语基符——那是第四纪教会用来为封印物建立'识别锚点'的标准铭文。而石碑释放的灵性频率恰好与这段基符产生了同频共振——这意味着石碑的建造者和最早封印物的制造者使用的是同一套赫密斯语体系。更关键的是——你检测到共振的方向不是单向的。石碑的辐射在响应封印物的同时，也在被封印物'回应'。这就像一个被叫醒的人在反问他旁边的人：你是谁？如果这个共振持续下去，石碑有可能会识别出你携带的封印物的弱点。",
      success:
        "你的灵性感知确认了三件封印物的异常不是巧合——它们的内部铭文都在回应同一个外部灵性信号。这个信号的频率与赫密斯语的仪式铭文结构完全吻合。这意味着异常源的材料和当代教会的封印物共享同一个第四纪知识源头。石碑和封印物在说同一种语言。",
      partial:
        "你能感知到三件封印物的灵性波动确实在同步变化——但原始信号的源头被石碑自身的灵性辐射遮蔽了。你能确认它们都在'回应'同一个东西，但无法精确定位回应的对象。要得到更清晰的判断，你需要降低周围的灵性干扰——或者找一个灵性感知更强的人。",
      failure:
        "石碑周围的灵性辐射太强了。你的灵性感知被不断干扰——封印物显示的方向在你眼中来回跳动，无法锁定。唯一能确认的是安魂铃的自鸣频率与感应纸的颜色变化似乎存在某种对应关系——但你无法确定它是真实的物理现象还是你的眼睛在昏暗的光线下产生的错觉。",
      catastrophe:
        "你在试图解码封印物的共振频率时，不慎让自己的灵性与石碑的灵性频率产生了接触。在那个瞬间——三件封印物的内部铭文同时发出了只有你一个人能听到的共鸣声。那不是声音——是你脑中的赫密斯语知识被石碑强行激活了。你在一瞬间'理解'了石碑想要的东西：它想被完成。污染+2。你猛地切断了灵性连接，但那个念头还在——像一颗种子。你无法确定自己得到的共振结果是真实的——还是石碑植入的。",
    },
  },
  {
    ...
    check: {
      ...
      baseDifficulty: 12,
      coopAllowed: true,
      assistAttributes: ["will", "aura"],
    },
    results: {
      revelation:
        "借阅登记簿上的墨迹涂抹掩盖不住一个细节：涂抹发生在三年前的深秋——恰好是旧教堂事件报告的提交日期之后的第三周。而涂抹的痕迹从页面边缘开始，逐渐向内延伸，方向显示借阅人的签名是用同一支笔、在同一天被登记上去然后被涂抹掉的。更关键的——你在文件夹的夹缝里发现了一张被对折夹着的值班表残片，上面有加尔文·塞特的签名和日期。日期就是旧案报告被标注'已处理'的前一天。不是所有文件都被销毁了——有人销毁了大部分，但遗漏了这一片。它刚好足够证明一件事：截留报告的指令来自值夜者总部内部。",
      success:
        "借阅登记簿上的涂抹痕迹说明有人事后修改了记录——但涂抹本身留下了时间线索。登记簿的纸质在墨迹浸透后产生了微弱的颜色变化，显示涂抹发生的时间是在原始记录登记之后不久。这意味着借走文件的人并不是事后才意识到自己的暴露——而是从一开始就知道自己不会归还。这不是疏忽。是故意截留。",
      partial:
        "你能找到借阅记录上的涂抹痕迹——但签名周围的墨迹受损太严重，辨认不出具体是谁。你只能确认一件事：三年前有人以'临时借阅'的名义取走了这批文件，并且再也没有归还。至于借阅人是谁——线索到此为止。需要更高权限的档案调阅。",
      failure:
        "档案室的照明很差，借阅登记簿上的字迹又小又潦草。你翻了十几页——其间至少有五条模糊的借阅记录可能和旧教堂事件有关，但你无法确认。档案室管理员在你身后不停咳嗽，他的紧张让你更难集中注意力。你就是找不到那个名字。",
      catastrophe:
        "你在翻阅借阅登记簿的时候，突然产生了一种强烈的被监视感。你回头看——身后只有一排灰扑扑的档案架和头顶一盏跳动的煤油灯。但你继续翻看登记簿时，发现了一件事：三年前那条被涂抹的借阅记录——刚才似乎不是这个位置的。你记不清它原本在第几页。不是记不清——是你的记忆被涂改了。你短时间内两次试图确认同一行记录，第二次看到的内容和第一次不一样。污染+2。你的证据变成不可靠的——因为你不知道自己看到的登记簿是不是真实的。",
    },
  },
  {
